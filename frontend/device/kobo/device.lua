...
local function check_unexpected_wakeup()
    ...
    logger.err("Kobo suspend: putting device back to sleep. unexpected wakeup count:",
...
function Kobo:suspend()
    ...
    -- If unexpected_wakeup_count is greater than 0, that means an unexpected
    -- wakeup already happened, so we keep the couter going.
    if unexpected_wakeup_count == 0 then unexpected_wakeup_count = 1 end
    -- assuming Kobo:resume() will be called in 10 seconds
    require("ui/uimanager"):scheduleIn(10, check_unexpected_wakeup)
end
